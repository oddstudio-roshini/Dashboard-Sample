...
/**
 * Branch detail page — shows branch info + doctors table
 * Doctor columns: Hospital, Doctor Name, Specialization, Contact Number,
 *                 Email ID, Highest Qualification, Consultation Fee, Registration Number
 */

import { useState, useEffect, useCallback } from 'react';
import { useRouter, useParams } from 'next/navigation';
import {
  Search, Stethoscope, MapPin, Phone, Mail,
  ChevronRight, ChevronLeft, Activity, Clock,
  Award, Hash, IndianRupee, ExternalLink,
} from 'lucide-react';

function googleMapsUrl(name: string, address: string, city: string) {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
    [name, address, city, "India"].filter(Boolean).join(", ")
  )}`;
}
import { clinicsApi } from '@/lib/clinicsApi';
import type { Branch, ClinicDoctor } from '@/types/clinics';
...
export default function BranchPage() {
  ...
  useEffect(() => {
    clinicsApi.getBranch(branchId).then(setBranch).catch(() => {});
  }, [branchId]);

  const fetchDoctors = useCallback(async (s: string) => {
    setLoading(true);
    try {
      const data = await clinicsApi.getDoctors(branchId, {
        search: s || undefined,
      });
      setDoctors(data);
    } catch {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 flex-wrap">
        ...
        <button onClick={() => router.push(`/dashboard/clinics/${clinicId}`)} className="hover:text-blue-600 transition-colors">
          {branch?.hospitalName || 'Hospital'}
        </button>
        <span>/</span>
        <span className="text-gray-900 font-medium">{branch?.branchName || 'Branch'}</span>
      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-500 mt-2">
                <div className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-gray-400" />{branch.address}, {branch.city}</div>
                <div className="flex items-center gap-1.5"><Phone className="w-4 h-4 text-gray-400" />{branch.contactPhone}</div>
                <div className="flex items-center gap-1.5"><Mail className="w-4 h-4 text-gray-400" />{branch.contactEmail}</div>
              </div>
              <p className="text-sm text-gray-600 mt-2"><span className="font-medium">Manager:</span> {branch.managerName}</p>
              <a
                href={googleMapsUrl(branch.branchName, branch.address || "", branch.city || "")}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-3 inline-flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 font-medium px-3 py-1.5 rounded-lg border border-blue-200 hover:bg-blue-50 transition-colors"
              >
                <MapPin className="w-3.5 h-3.5" /> View on Google Maps
              </a>
            </div>
            <div className="text-center flex-shrink-0">
              <p className="text-2xl font-bold text-purple-600">{branch.totalDoctors}</p>
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="text-base font-semibold text-gray-900">Doctors</span>
              ...
              <tbody className="divide-y divide-gray-50">
                {doctors.map((d, i) => (
                  <tr
                    ...
                    onClick={() => router.push(
                      `/dashboard/clinics/${clinicId}/hospitals/${hospitalId}/branches/${branchId}/doctors/${d.id}`
                    )}
                  >
                    <td className="px-4 py-3.5 text-gray-400 font-mono text-xs">{i + 1}</td>
                    <td className="px-4 py-3.5 text-gray-600 text-xs whitespace-nowrap">{d.hospitalName}</td>
